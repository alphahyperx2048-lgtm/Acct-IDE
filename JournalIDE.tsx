...
import { 
  Plus, Trash2, RotateCcw, Code, Undo2, Redo2, 
...
const JournalIDE: React.FC<{ setView?: (view: ViewMode) => void }> = ({ setView }) => {
  ...
  useEffect(() => {
    ...
    currentLines.forEach(line => {
      ...
      if (acc) {
        if (acc.type === 'ASSET' && line.type === 'CREDIT' && line.amount > getAccountBalance(acc.id)) {
          newWarnings.push(`PRUDENCE_ALERT: Asset "${acc.name}" credit exceeds current balance.`);
        }
      }
    });
    setWarnings(newWarnings);
  }, [currentLines, getAccountByName, getAccountBalance]);
  ...
  // Logic to suggest account type based on context
      ...
      <div className="h-14 md:h-16 border-b border-[#ffffff10] flex items-center justify-between px-3 md:px-6 bg-[#0a0f1c] shrink-0 z-50">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <button onClick={handlePost} className="flex items-center gap-1.5 md:gap-2 px-3 md:px-6 py-1.5 md:py-2.5 bg-[#00f3ff08] border border-[#00f3ff40] text-[#00f3ff] text-[9px] md:text-[11px] font-bold rounded-lg md:rounded-xl hover:bg-[#00f3ff] hover:text-slate-950 transition-all group">
            <Save size={12} className="group-hover:scale-110" /> <span className="uppercase tracking-widest">Post</span>
          </button>
        </div>
      </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-3 md:px-10 py-6 md:py-12">
          <div className="max-w-4xl mx-auto">
                              ...
                              {filteredSuggestions.map((acc) => (
                                <div key={acc.id} onClick={() => handleSelectAccount(line.id, acc)} className="px-4 py-3 cursor-pointer hover:bg-white/5 flex justify-between items-center group transition-colors border-b border-white/5 last:border-0">
                                  <div className="flex flex-col">
                                    ...
                                    <span className="text-[8px] md:text-[9px] text-gray-600 uppercase tracking-widest">{acc.code}</span>
                                  </div>
                                  <span className={`text-[7px] md:text-[8px] font-black px-1.5 py-0.5 rounded border ${acc.type==='ASSET'?'text-emerald-400 border-emerald-500/20':'text-orange-400 border-orange-500/20'}`}>{acc.type}</span>
                                </div>
                              ))}
                            </motion.div>
                          )}
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-8 md:mb-12 px-1 md:px-2">
              ...
              <button onClick={handleAutoBalance} className="w-full sm:w-auto px-4 md:px-5 py-2 md:py-2.5 bg-[#00f3ff08] border border-[#00f3ff20] rounded-lg md:rounded-xl text-[8px] md:text-[10px] font-black text-[#00f3ff] hover:bg-[#00f3ff10] transition-all flex items-center justify-center gap-1.5 md:gap-2 uppercase tracking-widest">
                <Sparkles size={10} className="hidden xs:inline" /> Auto-Balance ({difference.toFixed(1)})
              </button>
            </div>
            ...
            <div className="space-y-3 md:space-y-5 px-1 md:px-2">
              ...
              <textarea 
                ...
                placeholder="Enter transaction details (Being...)" 
              />
            </div>
            ...
            <div className="mt-12 md:mt-16 flex flex-col sm:flex-row justify-end gap-6 sm:gap-16 md:gap-24 border-t border-white/5 pt-8 md:pt-12 px-2">
               <div className="text-right flex flex-col gap-1">
                  ...
                  <div className="text-2xl md:text-5xl font-mono font-black text-[#10b981] drop-shadow-[0_0_15px_rgba(16,185,129,0.2)] tracking-tighter">
                    {totalDebit.toFixed(2)}
                  </div>
               </div>
               <div className="text-right flex flex-col gap-1">
                  <div className="text-[8px] md:text-[10px] text-gray-600 font-black uppercase tracking-[0.2em]">TOTAL CREDIT</div>
                  <div className="text-2xl md:text-5xl font-mono font-black text-[#f59e0b] drop-shadow-[0_0_15px_rgba(245,158,11,0.2)] tracking-tighter">
