...
import { 
  ...
  CircleDollarSign, Activity, Warehouse, CreditCard, 
  FileText, ShoppingBag, Receipt, Save
} from 'lucide-react';
...
const AccountRegisterModal: React.FC<AccountRegisterModalProps> = ({ isOpen, onClose, accountName, onSuccess, suggestedType, suggestedClass }) => {
  ...
  const handleRegister = () => {
    ...
    const newAcc = createAccount(accountName, selectedType, classification);
    onSuccess(newAcc);
    onClose();
  };
      ...
      >
        <div className="p-8 pb-4 flex items-center justify-between">
           <div className="flex items-center gap-4">
             ...
             <h2 className="text-2xl font-black text-white tracking-tighter uppercase">{accountName}</h2>
           </div>
